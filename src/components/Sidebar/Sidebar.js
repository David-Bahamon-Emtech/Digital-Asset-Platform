...
import EmtechLogo from '../Logo/EmtechLogo'; // Import the image logo component

// Sidebar navigation items
const sidebarItems = [
  { id: 'token-mgmt', label: 'Token Management' },
  { id: 'account-mgmt', label: 'Account Management' },
    ...
    <div className="w-64 bg-sidebar-bg text-sidebar-text h-screen fixed top-0 left-0 flex flex-col">
      ...
      <nav className="flex-grow px-4 -mt-11">
        ...
        {sidebarItems.map(item => (
          <div
            key={item.id}
            // Apply styling for padding, margins, cursor, active/hover states
            className={`py-2 px-4 -mx-4 mt-1 mb-1 cursor-pointer rounded ${
              activeTab === item.id
