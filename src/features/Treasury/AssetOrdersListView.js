  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <button className="px-3 py-1 rounded text-white hover:opacity-90 bg-gray-800 text-sm" onClick={onBack} > &larr; Back to Treasury Dashboard </button>
      </div>
      <div className="mb-6 text-right">
        <button className="px-4 py-2 rounded text-white hover:opacity-90 bg-green-600 text-sm font-medium" onClick={onCreateNew} > + Create New Order </button>
      </div>
      <div className="bg-white p-4 rounded shadow overflow-x-auto">
        {assetOrders.length === 0 ? ( <p className="text-center text-gray-500 italic">No asset orders found.</p> ) : (
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              ...
              {assetOrders.sort((a, b) => b.timestamp - a.timestamp).map((order) => (
                ...
                <tr key={order.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 whitespace-nowrap text-gray-500 max-w-xs truncate" title={`${order.from} -> ${order.to}`}> {order.from} &rarr; {order.to} </td>
                  <td className="px-4 py-2 whitespace-nowrap text-gray-500">{order.requestedBy}</td>
                  <td className="px-4 py-2 whitespace-nowrap text-center"> <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusClass(order.status)}`}> {order.status} </span> </td>
                </tr>
                // Ensure no whitespace
              ))}
              {/* Ensure no whitespace */}
